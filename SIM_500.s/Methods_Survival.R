...
SIPTW.aft=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv){
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      ...
      M=survreg(formula = Y ~ A, data=data.frame(Y,A),weights = weights, dist="weibull");betahat_T[i]=-M$coef[4]/M$scale}    
    Bias=mean(betahat_T)-ATT.surv[j]
    Variance=sum((betahat_T-mean(betahat_T))^2)/(Nsim-1)
    MSE=mean((betahat_T-ATT.surv[j])^2)
    table[[j]]=round(cbind(Bias,Variance,MSE),3)}
...
##### JFM1
JFM1.aft=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv, D, DD, Index_noC){
  registerDoParallel(16)
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      set.seed(i)
...
####### JIPTW1
JIPTW1.aft=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv, D, DD, Index_noC){
  registerDoParallel(16)
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      X=X_gen(nT,nC,nHC0,nHC1)
...
SFM=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv){
  ...
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      ...
      weights_HC1=m.out_HC1$weights[-c(1:nT)]
      weights=as.vector(c(rep(1,nT+nC),weights_HC0,weights_HC1))
      betahat_T[i]=summary(coxph(formula = Y ~ A, weights = weights, robust = TRUE))$coef[3]}  
    Bias=mean(betahat_T)-ATT.surv[j]
    Variance=sum((betahat_T-mean(betahat_T))^2)/(Nsim-1)
    MSE=mean((betahat_T-ATT.surv[j])^2)
    table[[j]]=round(cbind(Bias,Variance,MSE),3)}
...
##### JFM1
JFM1=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv, D, DD, Index_noC){
  registerDoParallel(16)
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      X=X_gen(nT,nC,nHC0,nHC1)
...
####### JIPTW1
JIPTW1=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv, D, DD, Index_noC){
  registerDoParallel(16)
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      X=X_gen(nT,nC,nHC0,nHC1)
...
JFM2=function(X, nT, DA, A, N, Index_T_HC0, Index_T_HC1, ATT.surv, D, DD, Index_noC){
  ...
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      ...
      weights_HC01=m.out$weights[-c(1:nT)]
      weights=as.vector(c(rep(1,nT+nC),weights_HC01))
      betahat_T[i]=summary(coxph(formula = Y ~ A + DD[,2] + DD[,3], weights = weights, robust = TRUE))$coef[3]}  
    Bias=mean(betahat_T)-ATT.surv[j]
    Variance=sum((betahat_T-mean(betahat_T))^2)/(Nsim-1)
    MSE=mean((betahat_T-ATT.surv[j])^2)
    table[[j]]=round(cbind(Bias,Variance,MSE),3)}
...
NPD=function(nT, DA, A, N, ATT.surv, NPD.surv, trt, extD){
  ...
  for(j in 1:4){
    betahat_T=foreach (i=1:Nsim, .combine=rbind) %dopar% {
      ...
      jags.out <- jags.model(name,data=list(N=N,Y=Y,event=event,trt = trt,extD = extD),
